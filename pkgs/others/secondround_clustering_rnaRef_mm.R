...
#rm(meta_path)
metadat = metadat %>%
  dplyr::select(-unharm_clusters, firstround_cluster = seurat_clusters)
...
p3pred <- DimPlot(coembed.harm, group.by = "annot", raster=FALSE,
                  split.by = "tech", label = TRUE, repel = TRUE, pt.size=.1) + ggtitle("Coembedding RNA and ATAC")
...
pdf(paste0(args$outfile, '_rnaToatac_celltype_confusion.matrix.pdf'), 10, 6)
pheatmap(tb,
...
p3adj <- DimPlot(seurat.harm, reduction = "umap", group.by = "celltype.adj",
                 label = TRUE, repel = TRUE, pt.size = .1) + ggplot2::ggtitle("Cell type corrected by majority vote")

p4adj <- DimPlot(seurat.harm, reduction = "umap", group.by = "predicted.id", 
              pt.size = .1, label = TRUE, repel = TRUE) + ggplot2::ggtitle("Original predicted cell type")
