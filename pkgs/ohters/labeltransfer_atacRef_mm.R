...
ref_feature = rownames(ref_counts)[rowSums(ref_counts) >= 50]
scidrop_feature = rownames(scidrop_counts)[rowSums(scidrop_counts) >= 50]
...
seurat_ref <- NucleosomeSignal(object = seurat_ref)
seurat_ref$nucleosome_group <- ifelse(seurat_ref$nucleosome_signal > 1, 'NS > 1', 'NS < 1')
print(table(seurat_ref$nucleosome_group))
...
fragfig = FragmentHistogram(object = seurat_ref,
                            region = "chr1-1-150000000")
pdf(paste0(args$outfile, "_atacToatac_fraghist_sciATAC_ref.pdf"), 6, 6)
print(fragfig)
dev.off()
...
seurat_ref <- TSSEnrichment(object = seurat_ref, tss.positions = tss_ranges)

p.tss <- VlnPlot(seurat_ref, features = "TSS.enrichment",
                 pt.size = 0) &
  geom_hline(yintercept = 2, linetype="dashed")

p.subnuc <- VlnPlot(seurat_ref, features = "nucleosome_signal",
                    pt.size = 0)

pdf(paste0(args$outfile, "_atacToatac_qc_plot_sciATAC_ref.pdf"), 14, 8)
print(wrap_plots(p.tss, p.subnuc, ncol = 2))
dev.off()

message('Saving ref tss scores...')
write_tsv(seurat_ref@meta.data %>% rownames_to_column("Barcodes"), paste0(args$outfile, '_atacToatac_sciATAC_ref_tss.txt'))

##################################################
ref_cells = intersect(rownames(ref_meta), colnames(ref_counts))
seurat_ref = subset(seurat_ref, cells = ref_cells)
ref_meta_filter = ref_meta[colnames(seurat_ref), ]
message('Check cells in metadata and seurat obj ...')
print(all(rownames(ref_meta_filter) == colnames(seurat_ref)))

seurat_ref = AddMetaData(seurat_ref, ref_meta_filter)
...
pref1 <- DimPlot(seurat_ref, reduction = "umap", group.by = "tissue.replicate", pt.size=.1)
pref2 <- DimPlot(seurat_ref, reduction = "umap", group.by = "cell_label", 
                 label = TRUE, repel = TRUE, pt.size=.1)
pref3 <- DimPlot(seurat_ref, reduction = "umap", pt.size=.1)

pdf(paste0(args$outfile, "_atacToatac_umap_sciATAC_ref.pdf"), 8, 6)
print(pref1)
print(pref2)
print(pref3)
dev.off()
...
saveRDS(seurat_ref, file = paste0(args$outfile, '_atacToatac_seurat.obj_sciATAC_ref.rds'))
...
seurat_scidrop <- MapQuery(
  ...
  reference = seurat_ref,
  query = seurat_scidrop,
  refdata = seurat_ref$cell_label,
  reference.reduction = "lsi",
  new.reduction.name = "reflsi",
  reduction.model = 'umap'
)

p1 <- DimPlot(seurat_ref, reduction = "umap", group.by = "cell_label", 
              label = TRUE, repel = TRUE, pt.size=.1) + NoLegend() + ggtitle("Reference sciATAC")
...
seurat_ref$dataset <- "sciATAC"
seurat_scidrop$dataset <- "sciDrop"
seurat_ref$predicted.id = seurat_ref$cell_label
  
# merge
combined <- merge(seurat_ref, seurat_scidrop)
