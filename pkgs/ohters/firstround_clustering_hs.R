...
# make seurat obj
peaks <- StringToGRanges(regions = rownames(counts), sep = c("_", "_"))

# a bug for ucsc that paralyzes GenomeInfoDb
# Solution: Instead of calling Seqinfo(genome="hg38) which is the default of seurat
# we call SeqinfoForUCSCGenome("hg38")
...
abline(v=log10(min_cell),lwd=2,col="indianred")
...
col <- c("#f3766e", "#11b4e9", "#9488c1", "#D39200", "#00C19F", "#93AA00", "#FF61C3")
...
seurat.singlet <- FindTopFeatures(seurat.singlet, min.cutoff = min_cell)
...
if (!is.null(args$hvar)) {
  ...
  cells = vector(mode = "list", length = length(levels(Idents(seurat.singlet))))
  names(cells) = levels(Idents(seurat.singlet))
  
  for (i in 1:length(cells)) {
    cells[[i]] = data.frame(cells = WhichCells(seurat.singlet, idents = i-1),
                            clusters = paste0("cluster", i-1))
  }
  ...
  message('check cell number extracted from each cluster')
  print(all(cell_count$Count == table(Idents(seurat.singlet))))
