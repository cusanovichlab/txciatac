...
if __name__ == '__main__':
    ...
    if args.nextseq:
        ...
        with open('/whitelist/scidrop_atac_10x_bc_nextseq.mismatch.pickle', 'rb') as bc10x:
            gem_correction_map = pickle.load(bc10x)
    else:
        with open('/whitelist/scidrop_atac_10x_bc.mismatch.pickle', 'rb') as bc10x:
            gem_correction_map = pickle.load(bc10x)

    sample_correction_map = construct_mismatch_to_whitelist_map(sample_idx_whitelist, 2)
    # gem_correction_map = construct_mismatch_to_whitelist_map(pcr_i7_whitelist, 2)
